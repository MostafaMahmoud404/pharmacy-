...
const getDoctors = asyncHandler(async (req, res, next) => {
  ...
  // Fix: Get proper count with filters
  const countQuery = Doctor.find(baseQuery);
  if (features.queryObj) {
    Object.keys(features.queryObj).forEach((key) => {
      if (
        key !== "page" &&
        key !== "limit" &&
        key !== "sort" &&
        ...
        countQuery.where(key).equals(features.queryObj[key]);
      }
    });
  }
...
const getDoctorById = asyncHandler(async (req, res, next) => {
  ...
  if (Review.getRatingDistribution) {
    ratingDistribution = await Review.getRatingDistribution(
      doctor._id,
      "doctor"
    );
  }
...
const toggleAvailability = asyncHandler(async (req, res, next) => {
  ...
  if (typeof isAvailable !== "boolean") {
    return next(new ErrorResponse("يرجى إرسال قيمة صحيحة للحالة", 400));
  }
...
const getUpcomingConsultations = asyncHandler(async (req, res, next) => {
  ...
  let consultations;
  if (Consultation.getUpcomingConsultations) {
    consultations = await Consultation.getUpcomingConsultations(doctor._id, 24);
  } else {
...
const rejectDoctor = asyncHandler(async (req, res, next) => {
  ...
  successResponse(res, 200, "تم رفض حساب الطبيب", { doctor, reason });
});
...
const searchDoctors = asyncHandler(async (req, res, next) => {
  ...
  const isDevelopment = process.env.NODE_ENV === "development";
  let query = isDevelopment ? {} : { isVerified: true, isAvailable: true };
  ...
  if (q && q.length >= 2) {
    ...
    if (userIds.length > 0) {
      ...
      return successResponse(res, 200, "نتائج البحث", {
        doctors: [],
        count: 0,
      });
    }
...
const getDoctorsBySpecialty = asyncHandler(async (req, res, next) => {
  ...
  const query = isDevelopment
    ? { specialty }
    : { specialty, isVerified: true, isAvailable: true };
